...
jQuery(function ($) {
  function Budget() {
    ...
    _.aSelect2.push({
      field: 'client_id',
      pathname: '/api/client/search'
    });

    _.aSelect2.push({
      field: 'payment_condition_id',
      pathname: '/api/payment-condition/search'
    });
  ...
  Budget.prototype.calcRowProduct = function (el) {
    var _ = this,
        ...
        nDiscount = $(document).find("input[name='discount[]']").eq(rowId).val(); //Atribui os valores as variaveis padrão


    nAmount = !nAmount ? 0.00 : nAmount;
    nUnitary_value = !nUnitary_value ? 0.00 : nUnitary_value;
    nTotal_value = !nTotal_value ? 0.00 : nTotal_value;
    nDiscount = !nDiscount ? 0.00 : nDiscount; //Converte todas as variaveis para string

    nAmount = nAmount.toString();
    nUnitary_value = nUnitary_value.toString();
    nTotal_value = nTotal_value.toString();
    nDiscount = nDiscount.toString(); //Formata todas as variaveis
    ...
    nUnitary_value = nUnitary_value.replace(',', '.');
    nTotal_value = nTotal_value.replace(',', '.');
    nDiscount = nDiscount.replace(',', '.'); //Transformas todas as variaveis em float

    nAmount = parseFloat(nAmount);
    nUnitary_value = parseFloat(nUnitary_value);
    nTotal_value = parseFloat(nTotal_value);
    nDiscount = parseFloat(nDiscount); //Calcula o valor unitario x quantidade

    nTotal_value = nAmount * nUnitary_value;
    nDiscount = nDiscount.toFixed(2);
    nTotal_value = nTotal_value.toFixed(2); //Verifica se o valor do desconto é maior que o item
    ...
    nTotal_value = parseFloat(nTotal_value); //Formata as casas decimais

    nAmount = nAmount.toFixed(2);
    nUnitary_value = nUnitary_value.toFixed(2);
    nDiscount = nDiscount.toFixed(2);
    nTotal_value = nTotal_value.toFixed(2); //Converte novamente para string os valores das variaveis 

    nAmount = nAmount.toString();
    nUnitary_value = nUnitary_value.toString();
    nDiscount = nDiscount.toString();
    nTotal_value = nTotal_value.toString(); //Formata os valores das variaveis

    nAmount = nAmount.replace('.', ',');
    nUnitary_value = nUnitary_value.replace('.', ',');
    nDiscount = nDiscount.replace('.', ',');
    nTotal_value = nTotal_value.replace('.', ','); //Atribui os valores calculados aos  campos
...
module.exports = __webpack_require__(/*! Z:\sistema\resources\js\budget.js */"./resources/js/budget.js");
